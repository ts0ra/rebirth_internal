...
namespace esp
{
	...
	void drawBox()
	{
		...
		ImVec2 topLeft = ImVec2(dataESP::headScreenPos.x - width / 2, dataESP::footScreenPos.y);
	...
	void drawHealth()
	{
		...
		ImVec2 topLeftBackground = ImVec2(dataESP::headScreenPos.x - width / 2 - healthBarWidth - 2, dataESP::footScreenPos.y);
		ImVec2 bottomRightBackground = ImVec2(dataESP::headScreenPos.x - width / 2 - healthBarWidth, dataESP::footScreenPos.y);

		ImU32 backgroundColor = ImColor(0.0f, 0.0f, 0.0f, 0.25f);
	...
	void drawDistance()
	{
		double distance = dataESP::targetPlayer->headPos.squaredDistance(data::game::localPlayer->headPos);
